...
def make_files(data):
    for key, value in data.items():
        ...
        period_coast_penny = str(value[1])[-2:]
        correct_ruble_case_for_year = get_correct_ruble_case(year_coast)
        correct_ruble_case_for_period = get_correct_ruble_case(period_coast_rub)
        correct_penny_case_for_period = get_correct_penny(period_coast_penny)
        ...
        extracted_text = extracted_text.replace("{РУБ_Р}", correct_ruble_case_for_period)
        extracted_text = extracted_text.replace("{КОПЕЙКИ}", period_coast_penny)
        extracted_text = extracted_text.replace("{КОП_Р_СКЛ}", correct_penny_case_for_period)
        extracted_text = extracted_text.replace("{СУММ_Г}", year_coast)
        extracted_text = extracted_text.replace("{ПРОПИСЬ_Г}", year_coast_str)
        extracted_text = extracted_text.replace("{РУБ_Г}", correct_ruble_case_for_year)
